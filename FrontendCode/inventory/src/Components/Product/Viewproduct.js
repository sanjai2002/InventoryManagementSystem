          ...
          data.map((d, i) => (
            d.stock <50 ? <>
              <tr key={i} className="blink">
                ...
                <td>{d.stock}</td>
                <td>
                 <Link to={`/Updateproduct/${d.productsId}`} className="btn btn-primary m-2">Update</Link> 
                <button onClick={e => Removeproduct(d.productsId)} className="btn btn-danger">Remove</button></td>
              </tr>
            </> :
             d.expiryDate == dateString ||d.expiryDate == dateString1? <>
              <tr key={i} className="blinkDate">
                ...
                <td>{d.expiryDate}</td>
                <td>{d.stock}</td>
                <td><Link to={`/Updateproduct/${d.productsId}`} className="btn btn-primary m-2">Update</Link>
                <button onClick={e => Removeproduct(d.productsId)} className="btn btn-danger">Remove</button></td>
              </tr>
            </> :<>
              <tr key={i}>
               ...
               <button onClick={e => Removeproduct(d.productsId)} type="button" class="btn btn-danger">Remove</button></td> 
              </tr></>   
          ))
        }
      </tbody>
